...
/**
 * Implements an instance of the flexible job shop scheduling problem with travel time between machines, and with
 * Lagrange multipliers for relaxed machine capacity constraints. For a detailed problem description see the NgMPPS
 * technical report.
 * 
 * @author ahaemm
 * 
...
public class FJSSTTproblem {
	...
	/**
	 * The number of operations per job, array indices are jobs. The set of operations for job i is
	 * {0,...,mOperations[i]-1}; It is assumed that the operations are to be processed in the order 0,...,mOperations[i]
	 * - 1.
	 */
	...
	/**
	 * The number of time units, indexed by {0,...,mTimeUnits - 1}. Each operation beginning time is defined as the
	 * beginning of the corresponding time unit, and each completion time the end of the time unit.
	 */
	int timeSlots;

	/**
	 * The sets of alternative machines per operation. A key is an integer tuple (job,operation) in the form of
	 * "job-operation", the corresponding value is the *set* (i.e. every value is in there once) of alternative
	 * machines.
	 */
	final HashMap<String, List<Integer>> altMachines;

	/**
	 * The process times of operations on machines. The first index is the job, the second index is the operation, and
	 * the third index is the machine.
	 */
	 ...
	 * @param objective
	 */
	public FJSSTTproblem(final int jobs, final int[] operations, final int maxOperations, final int machines,
			final int timeslots, final HashMap<String, List<Integer>> altMachines, final int[][][] processTimes,
			final int[][] travelTimes, final int[] dueDates, final Objective objective, final int[] weights) {
	...
	public FJSSTTproblem(final int jobs, final int[] operations, final int maxOperations, final int machines,
		...
		this(jobs, operations, maxOperations, machines, timeslots, altMachines, processTimes, travelTimes, dueDates,
				objective, weights);
	...
	public SubproblemInstance createSubproblem(int job) {
		...
		for (int j = 0; j < getOperations()[job]; j++) {
			...
			final List<Integer> altMachines = getAltMachines(job, j); // FIXME: debug
		...
		return new SubproblemInstance(job, operations, machines, timeslots, altMachinesMapping,
				processTimes_subproblem, travelTimes, dueDate, jobWeight, getObjective());
	}

	/**
	 * Implements a dynamic programming algorithm to determine the minimum completion time of a job, considering
	 * different process times on machines and travel times between machines.
	 * 
	 * @param job
	...
	public int calcMinJobCompletionTime(int job) {
		...
		/*
		 * v[j][m] is the minimum completion time of operation j assigned to machine m.
		 */
		...
		for (int op = 1; op < operations[job]; op++) {
			...
			for (int opMachine : opAltMachines) {
				// v[op][opMachine] is minimium over machine(op-1) {v[op-1][machine(op-1)] + travel time
				...
				for (int previousOpMachine : previousOpAltMachines) {
					int new_v = v[op - 1][previousOpMachine] + travelTimes[previousOpMachine][opMachine]
							+ processTimes[job][op][opMachine];
		...
		// last stage, determine minimum v value for last operation, which is the minimum job completion time
	...
	/**
	 * Calculates average maximum slacks between operations of a job, and before the first operation. These slacks are
	 * job specific.
	 * 
	...
	/**
	 * Create random travel times "mTravelTimes" between machines for a production network consisting of 3 shops.
	 * Machines are randomly assigned to shops, similar to the drawing process for the soccer worldcup. Travel time
	 * within a shop is 0. Travel time between 2 shops is a random number within given limits. The triangle inequality
	 * for travel times is satisfied. Travel times are symmetric.
	 * 
	 * @param lowerLimit
	...
	public void createTravelTimes(double lowerLimit, double upperLimit, PrintWriter pw) {
		...
		// create random travel times between shops, satisfying the triangle inequality
			...
			for (int i = 0; i < 3; i++) {
				for (int j = 0; j < 3; j++) {
					if (i != j && j > i) {
						...
						travelTimesShops[j][i] = travelTimesShops[i][j]; // symmetric travel times
					}
				}
			}
	...
	/**
	 * Calculates a lower bound on makespan for a FJSS problem (no travel times are considered, machine capacity
	 * constraints are neglected). This means that for a job, all operations are assigned to machines minimising process
	 * time, and the begin time for operation (j+1) = completion time for operation (j) + 1.
	 */
	public int calcLBmakespanFJSS() {
		...
		// identify maximum over the minimum job completion times: this is the makespan
