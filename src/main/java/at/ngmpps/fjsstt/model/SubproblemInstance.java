...
/**
 * Specifies a subproblem instance of the one job scheduling problem with marginal cost for time units on machines.
 * 
 * @author ahaemm
 * 
 */
...
public class SubproblemInstance {
	...
	/**
	 * The number of operations. The set of operations is {0,...,mOperations-1}; It is assumed that the operations are
	 * to be processed in the order 0,...,mOperations - 1.
	 */
	int operations;
	...
	/**
	 * The sets of alternative machines per operation. A key is an operation, the corresponding value is the set of
	 * alternative machines.
	 */
	HashMap<Integer, List<Integer>> altMachines;

	/**
	 * The process times of operations on machines. The first index is the operation, the second index is the machine.
	 */
	int[][] processTimes;
	...
	/**
	 * The Lagrange multipliers, i.e. the marginal cost for time units on machines. The first index is the machine, the
	 * second index is the time unit.
	 */
	double[][] multipliers;
	...
	public SubproblemInstance(final int id, final int operations, final int machines, final int timeslots,
			final HashMap<Integer, List<Integer>> altMachines, final int[][] processTimes, final int[][] travelTimes,
			final int dueDate, final int jobWeight, final Objective objective) {
		this.jobID = id;
	...
	/**
	 * Calculate the objective value as function of the last operation's completion time.
	 * 
	 * @param objective
	 *            The objective function for a single job. Completion time or tardiness.
	 * @param beginTime
	 *            The beginning time of the last operation, i.e. the operation (mOperation - 1).
	 * @param machine
	 *            The machine assigned to the last operation.
	 * 
	 * @return The objective value. The default value is the last operation's completion time.
	 */
	public double calcObjectiveValue(final Objective objective, final int beginTime, final int machine) {
		...
		case TARDINESS:
			...
			if (dueDate <= 0) {
				System.err
						.println("WARNING: job due date not set, but objective is tardiness. Returning job completion time.");
				return compTime;
			}
	...
	/**
	 * Calculates the total cost of a subproblem with a given solution (encoded in a bid) and with given Lagrange
	 * multiplier values. Total cost is the sum of objective value (e.g. tardiness) and utilisation costs of machines.
	 * 
	 * @param bid
	 *            The given solution.
	 * @param multipliers
	...
	/**
	 * Calculates the total cost of a subproblem, when using the augmented price adjustment. The total cost is the sum
	 * of the Lagrangian multipliers and the sum of the decision variables of a time zone squared times a scaling
	 * factor.
	 * 
	 * @param objective
	 * @param bid
	...
	/**
	 * Calculates random values for Lagrange multipliers from the interval (0,upperBound].
	 * 
	 * @param seed
	 * @param upperBound
